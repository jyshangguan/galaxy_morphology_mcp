...
2. 其次，细致分析星系所包含的成分类型。采用逐成分递增的方式进行，保证拟合的稳定性；成分添加逻辑
    1. 成分添加次序：先建立基础结构（Disk、Bulge），再添加细节成分（Bar、Nucleus）
        1. 比如：如果同时存在 Disk、Bulge、Nucleus；应该先添加尺寸最大的 Disk, 最后加尺寸最小的Nucleus
        2. 比如：如果预估有 Bar 的存在，首先要优先保障 Disk 和 Bulge 已经添加（Disk → Bulge → Bar）。有 Bar 必然有 Disk.
